...
export default function TripDetails() {
  ...
  async function getTripDetails() {
    try {
      const res = await axios.get(`http://veemanage.runasp.net/api/TripRequest/${id}`, {
        headers: {
  ...
  const tripDetails = [
    { title: 'Vehicle Name', value: data.tripRequest?.vehicle?.vehicleModelDto?.brand?.name || 'N/A' },
    { title: 'Driver Name', value: data.tripRequest?.driver?.displayName || 'N/A' },
    { title: 'Destination', value: data.tripRequest?.destination || 'N/A' },
    { title: 'Status', value: data.tripRequest?.status || 'N/A' },
    { title: 'Start Time', value: data.tripRequest?.startTime || 'N/A' },
    { title: 'Date', value: data.tripRequest?.date || 'N/A' },
    { title: 'Vehicle Category', value: data.tripRequest?.vehicle?.category?.name || 'N/A' },
    { title: 'License Plate', value: data.tripRequest?.vehicle?.palletNumber || 'N/A' },
  ];

  if (isLoading) {
    return (
      <div className="text-center py-4 text-stone-700">Loading trip details...</div>
    );
  }
    ...
    <>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 py-2 px-5 rounded-lg shadow-lg border border-stone-300 gap-4 mb-5 ">
        {tripDetails.map((detail, index) => (
          <div key={index} className="flex flex-col gap-2 justify-center items-start mb-4">
